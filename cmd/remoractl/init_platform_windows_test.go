...
func TestGenerateWindowsServiceInstaller(t *testing.T) {
	...
	script := string(data)
	for _, required := range []string{"#Requires -RunAsAdministrator", "--service -c", "NT SERVICE\\JellyfinRemora", "New-Service", "Set-ServiceIdentity", "Invoke-CimMethod", "failureflag", "Start-Service", "InstallTask", "StartTask", "Stop-ScheduledTask", "Register-ScheduledTask", "Installed scheduled task", "New-EventLog", "$installDir", "(OI)(CI)RX", "Grant-ServiceLogonRight", "LsaAddAccountRights", "SeServiceLogonRight"} {
