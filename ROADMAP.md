...
- [x] `v0.9.0-beta.5` binds user-selected Generic packages to the official Jellyfin repository before deployment. Kickstart visibly reports `Connecting Jellyfin repo` and, for archive-only releases, `Downloading package`; network connection, response, and download operations are bounded by timeouts. Published SHA-256 metadata is preferred, official-package download/hash comparison is the legacy fallback, and extraction rechecks the validated local bytes to close the replacement window.
- [x] `v0.9.0-beta.6` closes the first 2026-07-19 external-review round for HA state, process identity, storage fencing, and database-generation safety. Twenty-six accepted findings remain individually traceable; macOS arm64 plus Rocky and Debian amd64 passed PID adoption, storage loss/recovery, durable manual stop, legacy three-line state upgrade, and database-damage generation fencing. Windows process and Job Object changes passed unit and cross-build gates pending the native Windows release gate.
- [x] `v0.9.0-beta.7` closes the second 2026-07-19 external-review round for Kickstart, archive extraction, and initialization transactions. Eight accepted findings remain individually traceable. Archive expansion is bounded and digest-bound to one open file description; repository failures remain distinguishable from absent packages; repeated init preserves and backs up operator configuration; signal interruption removes sensitive temporary files; and failed installs unwind completed filesystem and service steps or print an exact cleanup manifest.
- [x] `v0.9.0-beta.8` closes the third 2026-07-19 external-review round for the local control plane and credential boundary. Fifteen accepted findings remain individually traceable. New configurations use socket-only control unless TCP is explicitly enabled; Unix discovery validates socket owner and peer UID; Windows removes Interactive Users from the pipe DACL; Jellyfin and control clients reject redirects; administrator-password requests require current process-generation port ownership; HTTP bodies and errors are bounded; queued actions, immediate healthchecks, persistence errors, configuration ownership, and rotated log descriptors now have explicit lifecycle semantics.
- [x] `v0.9.0-beta.9` closes the fourth 2026-07-19 external-review round for configuration, platform behavior, and test-contract consistency. Fourteen accepted findings remain individually traceable. Runtime controls and resolved paths are validated strictly; nullable HTTPS, environment-name casing, YAML scalar types, custom assets, XML durability, Darwin privilege dropping, Windows folder mounts, Unicode identifiers, and systemd path escaping now have deterministic contracts. macOS arm64 plus Rocky and Debian amd64 passed init, permissions, environment, launchd/systemd artifact, and ownership-preserving edit gates; Windows remains cross-build and seam tested pending its native release gate.
